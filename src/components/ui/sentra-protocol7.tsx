    ...
    <section aria-labelledby="figma-landing-title" className="bg-background py-16">
      <div className="container mx-auto px-6 text-center max-w-5xl">
        ...
        <p className="mt-4 text-[17px] leading-[1.8] font-medium text-gray-600 max-w-2xl mx-auto">
          Dibangun di atas <strong>Protokol 7</strong>, AADI menghadirkan alur{' '}
          <strong>end-to-end</strong> diagnosis hingga tatalaksana, teroptimasi untuk Fasilitas
            ...
            <Link href="#" aria-label="Documentation">
              <span className="inline-flex items-center gap-2">
                <span
                  ...
                  style={{
                    background:
                      'conic-gradient(from 0deg, #8b5cf6, #22c55e, #f59e0b, #ef4444, #3b82f6)',
                  }}
                />
                Documentation
              </span>
      ...
      <div className="container mx-auto px-6 mt-8 grid grid-cols-1 md:grid-cols-3 gap-5">
        {features.map((f, i) => (
          <div
            key={i}
            className="rounded-xl border border-border bg-white text-left p-6 flex flex-col gap-3"
          >
            <Image src={f.iconSrc} alt="" width={18} height={18} className="mt-[2px]" />
            <div>
              <h3
                className="text-sm font-semibold mb-2"
                style={{ fontFamily: 'var(--font-heading)', color: '#002147' }}
              >
                {f.title}
              </h3>
      ...
      <div className="container mx-auto px-6 mt-10 text-center">
        <p className="tracking-widest text-[11px] text-muted-foreground">
          ENDORSED BY IKATAN DOKTER INDONESIA KEDIRI
        </p>
      </div>
    </section>
  );
