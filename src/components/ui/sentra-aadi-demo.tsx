...
function DevDiagnostics() {
  ...
  tests.push({
    name: "Checks only contains 'Gejala'",
    pass: /Gejala: Febris, Fatigue\./.test(t2out),
    details: t2out,
  });
  ...
  const t3pass =
    /Gejala: Febris\./.test(t3out) &&
    /Status: Stabil/.test(t3out) &&
    /Rencana: Kontrol 1 minggu/.test(t3out) &&
    /Note bebas/.test(t3out) &&
    /Diagnosis Banding: DBD; Influenza/.test(t3out) &&
    /Diagnosis Utama: URTI/.test(t3out);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
      ...
      tests.forEach((t) =>
        console.warn(`${t.pass ? '✅' : '❌'} ${t.name}`, t.details ? `\n→ ${t.details}` : ''),
      );
      console.groupEnd();
    }
  }, [tests]);
...
export default function SentraAadiDemo() {
  ...
  const handleGenerate = () => {
    setNotes({
      ...
      summary:
        'Lanjutkan regimen saat ini; jadwal kontrol 1 minggu; edukasi hidrasi & monitoring gejala.',
      adherence:
        'Kepatuhan sekitar 90%; 1 dosis lupa dalam 7 hari terakhir, telah diberi pengingat harian.',
      pharmacogen:
        'PGx: CYP2C19 *1/*1 (NM), CYP2D6 *1/*2 (NM). Tidak perlu penyesuaian dosis untuk terapi saat ini.',
    });
  ...
  const clinicalSummary = useMemo(() => {
    if (checks.length === 0 && !freeSummary.trim() && !diffDiag.trim() && !mainDiag.trim())
      return '';
  ...
  function moveCursorTo(
    target: TargetKey,
    opts?: { offsetX?: number; offsetY?: number; center?: boolean },
  ) {
    const rect = targetMap[target]();
    if (!rect) return;
    const x = opts?.center
      ? rect.left + rect.width / 2
      : rect.left + (opts?.offsetX ?? rect.width * 0.8);
    const y = opts?.center
      ? rect.top + rect.height / 2
      : rect.top + (opts?.offsetY ?? rect.height * 0.6);
    setCursorPos({ x, y });
  }
  ...
  return (
    <div className="min-h-screen w-full bg-white pt-20 px-6 text-[13px] leading-[1.7] relative">
      ...
      <div
        className="mx-auto max-w-7xl w-full bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-300 relative"
        style={{ fontSize: '13px' }}
      >
        {/* Device Status Bar */}
          ...
          <div className="flex items-center gap-2">
            <button className="w-6 h-6 bg-gray-600 rounded flex items-center justify-center hover:bg-gray-500 transition-colors">
              <svg className="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fillRule="evenodd"
                  d="M7.707 3.293a1 1 0 010 1.414L5.414 7H11a1 1 0 110 2H5.414l2.293 2.293a1 1 0 11-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z"
                  clipRule="evenodd"
                />
              </svg>
            </button>
        ...
        <div className="bg-white p-6 space-y-5 relative">
          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-5 pb-8 relative">
            ...
            <section className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 flex flex-col gap-3 min-h-[260px] lg:col-span-1">
              <h2
                className="text-[14px] font-semibold text-gray-900 h2-underline"
                style={{ color: '#002147', fontFamily: 'var(--font-heading)' }}
              >
                PATIENT ID
              </h2>
              <Field label="Nama">
                <input className="input text-[13px]" defaultValue="Budi Santoso" />
              </Field>
              <Field label="No. Rekam">
                <input className="input text-[13px]" defaultValue="654321" />
              </Field>
              <Field label="Tanggal Lahir">
                <input className="input text-[13px]" defaultValue="03/15/1955" />
              </Field>
              <button
                ref={generateBtnRef}
                onClick={handleGenerate}
                className="w-full mt-3 rounded-lg py-3 text-white text-[13px] font-semibold shadow"
                style={{ backgroundColor: accent }}
              >
                Generate
              </button>
            ...
            <section
              ref={developmentCardRef}
              className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 flex flex-col gap-2 min-h-[260px] lg:col-span-1"
            >
              <h2
                className="text-[14px] font-semibold text-gray-900 h2-underline"
                style={{ color: '#002147', fontFamily: 'var(--font-heading)' }}
            ...
            <section className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 flex flex-col gap-3 min-h-[260px] lg:col-span-1">
              ...
              <textarea
                className="mt-2 w-full rounded-md border border-gray-300 p-3 text-[13px] focus:outline-none focus:ring-2"
                style={{ outline: 'none', accentColor: accent }}
                placeholder="Ringkasan bebas (opsional)..."
                rows={4}
                value={freeSummary}
                onChange={(e) => setFreeSummary(e.target.value)}
              />
            </section>
            ...
            <section className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 flex flex-col gap-3 min-h-[260px] lg:col-span-1">
              ...
              <Field label="Diagnosis Utama">
                <textarea
                  ...
                  rows={4}
                  value={mainDiag}
                  onChange={(e) => setMainDiag(e.target.value)}
                />
              </Field>
              ...
              {clinicalSummary && (
                <div
                  ref={summaryBoxRef}
                  className="mt-4 rounded-lg border p-4 text-[13px] leading-relaxed relative"
                  style={{ borderColor: oxford }}
                >
                  <div className="mb-2 flex items-center gap-3">
                    <span
                      ...
                      style={{ backgroundColor: accent }}
                    />
                    <span className="font-semibold text-[14px]" style={{ color: oxford }}>
                      Ringkasan Klinis
                    </span>
                  </div>
            ...
            <section className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-gray-200 flex flex-col gap-3 min-h-[260px] lg:col-span-1">
              ...
              <Field label="Dosage Instructions">
                <textarea
                  ...
                  rows={3}
                  defaultValue="Minum setelah makan. Hindari alkohol selama pengobatan."
                />
              </Field>
          ...
          <p
            className="text-center text-[13px] max-w-4xl mx-auto pb-8"
            style={{ color: 'rgba(0,33,87,0.60)' }}
          >
            Konten dan fitur dalam versi demo ini bukan produk akhir dan dapat berubah. Demo ini
      ...
      {showTutorial && (
        <>
          ...
          {tooltip && (
            ...
            >
              <div className="font-semibold mb-2" style={{ color: oxford, fontSize: '17px' }}>
                Tutorial
              </div>
